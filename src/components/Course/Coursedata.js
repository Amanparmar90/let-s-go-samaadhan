// Courselisteningdata.js

const Coursedata = [
    {
       id: 0,
     title: "Veg Cheez Manchurian | Vegetarian",
     instructors: ["Aman Parmar", "Ranveer Bara", "Kunal Kapoor"],
     details: [
       "Course duration: 2 years for main course and home meal",
       "Classes start from 1 Dec 2023, English + Hindi",
       "Online application support, morning and evening classes",
       "Sanjeev Kapoor, Ranveer Bara, Kunal Kapoor, and 7 more",
     ],
     price: "1,50,999/- only",
     button: "Get The Course",
    
   },
   {
       id: 1,
     title: "Veg Cheez Manchurian | Vegetarian",
     instructors: ["Nilesh Kumar", "Ranveer Bara", "Kunal Kapoor"],
     details: [
       "Course duration: 2 years for main course and home meal",
       "Classes start from 1 Dec 2023, English + Hindi",
       "Online application support, morning and evening classes",
       "Sanjeev Kapoor, Ranveer Bara, Kunal Kapoor, and 7 more",
     ],
     price: "1,50,999/- only",
     button: "Get The Course",
     
   },
    {
       id: 2,
     title: "Veg Cheez Manchurian | Vegetarian",
     instructors: ["Ojash", "Ranveer Bara", "Kunal Kapoor"],
     details: [
       "Course duration: 2 years for main course and home meal",
       "Classes start from 1 Dec 2023, English + Hindi",
       "Online application support, morning and evening classes",
       "Sanjeev Kapoor, Ranveer Bara, Kunal Kapoor, and 7 more",
     ],
     price: "1,50,999/- only",
     button: "Get The Course",
     
   },
    {
       id: 3,
     title: "Veg Cheez Manchurian | Vegetarian",
     instructors: ["TeJ Panchal", "Ranveer Bara", "Kunal Kapoor"],
     details: [
       "Course duration: 2 years for main course and home meal",
       "Classes start from 1 Dec 2023, English + Hindi",
       "Online application support, morning and evening classes",
       "Sanjeev Kapoor, Ranveer Bara, Kunal Kapoor, and 7 more",
     ],
     price: "1,50,999/- only",
     button: "Get The Course",
    
   },
   // Add more course objects as needed
];
 
 export default Coursedata;
 